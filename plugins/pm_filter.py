...
async def filter(client, message):
        ...
        try:
            ...
            await client.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("♻️ 𝙹𝚘𝚒𝚗 𝙼𝚢 𝙲𝚑𝚊𝚗𝚗𝚎𝚕", url=invite_link.invite_link)
                        ]
                    ]
                ),
                parse_mode="markdown"
    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>🗂️Total File :- {message.from_user.mention}</b>\n<b>🎥Movie Name :-</b> <code>{search}</code> ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))

            else:
                await message.reply_text(f"<b>🗂️Total File :- {message.from_user.mention} </b>\n<b>🎥Movie Name :-</b> <code>{search}</code> ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
